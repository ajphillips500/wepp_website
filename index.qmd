...
Permutation invariance is the property that for any finite set of time points, the joint distribution over those points is invariant under any permutation of their indices. This is similar to, but distinct from, exchangeability. The mathematical definition of exchangeability is that for all $k\in \mathbb{N}$ and for all permutations $\pi$ of $\{1,\dots, k\}$ and measureable sets $F_i\subseteq \mathbb{R}$

$$\mathbb{P}(X_{t_{\pi(1)}}\in F_1, \dots, X_{t_{\pi(k)}}\in F_k) = \mathbb{P}(X_{t_1}\in F_1,\dots, X_{t_k}\in F_k)$$

Whereas permutation invariance is defined as

$$\mathbb{P}(X_{t_{\pi(1)}}\in F_{\pi(1)}, \dots, X_{t_{\pi(k)}}\in F_{\pi(k)}) = \mathbb{P}(X_{t_1}\in F_1,\dots, X_{t_k}\in F_k)$$
