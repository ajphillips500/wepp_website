...
Let $s\subset z$ and $s'\subset z'$ be the subsets of points in clusters by themselves and $|s|$ be the cardinality of those sets. From the conditional distributions described on the "iMGPE" page, we have 
...
The $n+\alpha-1$ term will cancel from all terms. Then, Brook's lemma becomes
...
Each data set was generated in three steps. First, I drew 100 points from a $Unif(0,1)$ distribution as our $X$ input variable. Second, I drew three values $a$, $b$, and $c$ from the intervals $[0,3]$, $[-1,1]$, and $(0,2]$. These serve as parameters that define the mean function $g(\cdot)$ for the data set.
$$g(x)=(a+bx)I(x\leq 2) + \cos(c\pi x)I(x > 2)$$
Given this, I generated $Y_i=g(X_i)+\epsilon_i$ where the $\epsilon_i$ are independent $N(0,0.2^2)$ variables. This results in a number of data sets with discontinuities around 2.

The predictive RMSE for the two methods are compared below. You can see that the data sets fall into two groups: those where iMGPE-EM significantly outperformed iMGPE-Slice and those where it only slightly outperformed iMGPE-Slice.
...
$$p(y|z,\mu,\Sigma)=\prod_{j=1}^{J_z} N_{C_j}(\mu_j,\Sigma_j)$$
$$\mu_j,\Sigma_j\stackrel{ind}{\sim} G \text{ for } j=1,\dots,J_z$$
$$z\sim DP(\alpha G)$$
where $C_j$ is the number of points in cluster $j$ given $z$ and $J_z$ is the number of clusters.
...
$$y_t|\theta_t,\tau^2 \stackrel{ind}{\sim} N(\theta_t, \tau^2I_n), t=1,\dots,T$$
$$\theta_t\stackrel{iid}{\sim} G^{(n)}, t=1,\dots,T$$
$$G^{(n)}|\alpha,\sigma^2,\phi \sim DP(\alpha G_0^{(n)})$$
$$G_0^{(n)}(\cdot|\sigma^2,\phi) = N_n(\cdot|0,\sigma^2H_n(\phi))$$
where $t$ indexes the observations at each location and $G_0^{(n)}$ is a Gaussian process with covariance function parameterized by $\phi$ over our observed locations.
