...
I ran the Metropolis-Hastings sampler with two different sets of priors. First, I used the same priors as last week, $IG(1,1)$ on $\theta$ and $Ca^+(0,2)$ on $\tau$ and $\eta$. Then I applied the priors used by Lalchand and Rasmussen, lognormal $logN(0,3)$ priors for all three parameters.
...
The $\hat R$ values for all chains were below $1.001$, indicating that all chains had converged. In all cases, the Metropolis-Hastings estimates were reasonably close to the true parameter values and were close to the STAN estimates in almost all cases. STAN overestimated the spatial variance on the data set with $\tau=2.83$ and both methods underestimated the lengthscale for $\theta=5$ and $\theta=10$.
