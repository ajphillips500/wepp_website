...
I updated the priors of my Metropolis Hastings sampler as discussed in the previous section, then implemented an adaptive proposal variance. Proposals for new parameters are now drawn from a multivariate normal distribution with covariance matrix $2.4^2S/d$, where $d$ is the number of parameters and $S$ is the sampling variance of the parameters. $S$ is updated every $100$ iterations for the first thousand, based on samples already drawn. I also discarded only the first $1000$ iterations as burn-in.
...
The table below shows the estimates and standard errors for each parameter, averaged across the data sets.

```{r}
tab1 <- data.frame('Par'=c('Theta','Tau','Eta'), 'True'=c(0.5,1.414,0.707), 'MH_Est'=c(0.649,0.608,0.451), 'STAN_Est'=c(3.442,0.420,0.435), 'MH_SE'=c(0.009,0.02,0.016), 'STAN_SE'=c(0.475,0.041,0.016))
knitr::kable(tab1, digits = 3, caption = "Estimates and SE for Both Samplers")
```

The acceptance rates for the M-H sampler vary from $52\%$ to $2.4\%$, indicating that the adaptive proposal section was not always successful in reaching the optimal acceptance rate of $23\%$. Further tuning may be required.
...
This part of the project ran into difficulties and is not ready to be presented.
